module "vpc" {
  ...
  default_vpc_id = var.vpc["vpc_default_id"]
  default_vpc_rt = var.vpc["vpc_default_rt"]
  default_vpc_cidr = var.vpc["vpc_default_cidr"]   
}
 module "apps" {
    source = "./module/ec2"
   ...
   subnets_ids   = [module.vpc.public_subnets_ids[each.value["subnet_ref"]]]
